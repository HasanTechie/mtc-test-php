...
for ($i = 1; $i <= 10; $i++) { //Looping through all pages of api
    ...
    foreach ($response->data as $responseInstance) {
        foreach ($responseInstance as $key => $value) {
            $dataArray[$key] = $value; //Checking each field for SQL injections; One field contained Apostrophe(') which was breaking SQL code
        }

        unset($dataArray['property_type']);
        $property = new Property();
        $property->store($dataArray);
